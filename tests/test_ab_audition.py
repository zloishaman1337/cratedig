...
class TestABGUISmoke:
    ...
    def test_main_window_has_ab_state_attribute(self):
        """MainWindow exposes _ab_state attribute (or initializes it)."""
        self._app()
        pytest.importorskip("PySide6")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Should have an _ab_state attribute
            assert hasattr(window, "_ab_state")
            window.close()
            db.close()

    def test_main_window_has_ab_toggle_shortcut(self):
        """MainWindow has a keyboard shortcut to toggle A/B (e.g., 'X' or 'Tab')."""
        self._app()
        pytest.importorskip("PySide6")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Should have a toggle action or button with shortcut
            assert hasattr(window, "_ab_toggle_action") or hasattr(window, "_ab_toggle_btn")
            window.close()
            db.close()

    def test_main_window_set_ab_slot_a_callable_no_raise(self):
        """MainWindow.set_ab_slot_a(sample_id) is callable and doesn't raise."""
        self._app()
        pytest.importorskip("PySide6")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Should be callable with a sample id
            try:
                window.set_ab_slot_a(1)
            except AttributeError:
                # If method doesn't exist yet, we expect this in the failing test
                pytest.fail("MainWindow.set_ab_slot_a(sample_id) must exist")
            finally:
                window.close()
                db.close()

    def test_main_window_set_ab_slot_b_callable_no_raise(self):
        """MainWindow.set_ab_slot_b(sample_id) is callable and doesn't raise."""
        self._app()
        pytest.importorskip("PySide6")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Should be callable with a sample id
            try:
                window.set_ab_slot_b(2)
            except AttributeError:
                pytest.fail("MainWindow.set_ab_slot_b(sample_id) must exist")
            finally:
                window.close()
                db.close()

    def test_main_window_toggle_ab_slot_callable_no_raise(self):
        """MainWindow.toggle_ab_slot() is callable and doesn't raise."""
        self._app()
        pytest.importorskip("PySide6")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                window.toggle_ab_slot()
            except AttributeError:
                pytest.fail("MainWindow.toggle_ab_slot() must exist")
            finally:
                window.close()
                db.close()
